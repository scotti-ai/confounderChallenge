...
ax.scatter(dreamy_data['patient'], dreamy_data['post_surgical_score'],
           marker='o', color=color_dreamy, s=60, alpha=0.7, 
           label='Doc Dreamy', edgecolors='black', linewidths=0.5)
ax.scatter(duck_data['patient'], duck_data['post_surgical_score'],
           marker='^', color=color_duck, s=60, alpha=0.7,
           label='Doc Duck', edgecolors='black', linewidths=0.5)
...
x_annotation = x_max * 0.98

# Add text annotations with shape symbols
ax.text(x_annotation, mean_dreamy, f'○ {mean_dreamy:.2f}',
        color=color_dreamy, fontsize=11, ha='right', va='bottom',
        bbox=dict(boxstyle='round,pad=0.3', facecolor='white', 
                 edgecolor=color_dreamy, alpha=0.8))
ax.text(x_annotation, mean_duck, f'△ {mean_duck:.2f}',
        color=color_duck, fontsize=11, ha='right', va='top',
        bbox=dict(boxstyle='round,pad=0.3', facecolor='white', 
                 edgecolor=color_duck, alpha=0.8))
...
mean_dreamy <- patients_df %>%
  filter(doctor_name == 'Doc Dreamy') %>%
  summarise(mean = mean(post_surgical_score)) %>%
  pull(mean)

mean_duck <- patients_df %>%
  filter(doctor_name == 'Doc Duck') %>%
  summarise(mean = mean(post_surgical_score)) %>%
  pull(mean)
...
ggplot(patients_df, aes(x = patient, y = post_surgical_score, 
  ...
  # Horizontal dashed lines for means
  geom_hline(yintercept = mean_dreamy, color = color_dreamy, 
             linetype = 'dashed', linewidth = 0.8, alpha = 0.8) +
  geom_hline(yintercept = mean_duck, color = color_duck, 
             linetype = 'dashed', linewidth = 0.8, alpha = 0.8) +
  # Text annotations with shape symbols
  annotate('text', x = Inf, y = mean_dreamy, 
           label = paste0('○ ', round(mean_dreamy, 2)),
           color = color_dreamy, size = 3.5, hjust = 1.05, vjust = -0.3,
           fontface = 'bold') +
  annotate('text', x = Inf, y = mean_duck, 
           label = paste0('△ ', round(mean_duck, 2)),
           color = color_duck, size = 3.5, hjust = 1.05, vjust = 1.2,
           fontface = 'bold') +
  # Color and shape scales
  scale_color_manual(
    values = c('Doc Dreamy' = color_dreamy, 'Doc Duck' = color_duck),
    name = 'Doctor'
  ) +
  scale_shape_manual(
    values = c('Doc Dreamy' = 19, 'Doc Duck' = 17),  # 19 = circle, 17 = triangle
    name = 'Doctor'
  ) +
  # Professional styling
  labs(
...
# Define colors (same as Figure 1)
color_dreamy = '#4E79A7'  # blue
color_duck = '#E15759'    # coral/red-orange
...
# Get x-axis limits for annotation positioning
x_max = patients_randomized_df['patient'].max()
x_annotation = x_max * 0.98
...
# Calculate means for annotations
mean_dreamy <- patients_randomized_df %>%
  filter(doctor_name == 'Doc Dreamy') %>%
  summarise(mean = mean(post_surgical_score)) %>%
  pull(mean)

mean_duck <- patients_randomized_df %>%
  filter(doctor_name == 'Doc Duck') %>%
  summarise(mean = mean(post_surgical_score)) %>%
  pull(mean)

# Create the plot
ggplot(patients_randomized_df, aes(x = patient, y = post_surgical_score, 
                                    color = doctor_name, shape = doctor_name)) +
  # Scatter points
  geom_point(size = 3, alpha = 0.7, stroke = 0.5) +
  # Horizontal dashed lines for means
...
# Define colors (same as previous figures)
color_dreamy = '#4E79A7'  # blue
color_duck = '#E15759'    # coral/red-orange
...
ax.scatter(dreamy_data['severity'], dreamy_data['post_surgical_score'],
           marker='o', color=color_dreamy, s=60, alpha=0.7, 
           label='Doc Dreamy', edgecolors='black', linewidths=0.5, zorder=2)
ax.scatter(duck_data['severity'], duck_data['post_surgical_score'],
           marker='^', color=color_duck, s=60, alpha=0.7,
           label='Doc Duck', edgecolors='black', linewidths=0.5, zorder=2)
...
x_min = patients_df['severity'].min()
x_range = x_max - x_min
x_annotation = x_max - x_range * 0.02

# Add text annotations with shape symbols
ax.text(x_annotation, mean_dreamy, f'○ {mean_dreamy:.2f}',
        color=color_dreamy, fontsize=11, ha='right', va='bottom',
        bbox=dict(boxstyle='round,pad=0.3', facecolor='white', 
                 edgecolor=color_dreamy, alpha=0.8), zorder=3)
ax.text(x_annotation, mean_duck, f'△ {mean_duck:.2f}',
        color=color_duck, fontsize=11, ha='right', va='top',
        bbox=dict(boxstyle='round,pad=0.3', facecolor='white', 
                 edgecolor=color_duck, alpha=0.8), zorder=3)
...
# Get y-axis range for shaded region
y_min <- min(patients_df$post_surgical_score)
y_max <- max(patients_df$post_surgical_score)
y_range <- y_max - y_min

# Create the plot
ggplot(patients_df, aes(x = severity, y = post_surgical_score, 
                        color = doctor_name, shape = doctor_name)) +
  # Shaded rectangular overlay highlighting severity between -1 and 1
  annotate('rect', xmin = -1, xmax = 1, ymin = -Inf, ymax = Inf,
           alpha = 0.15, fill = 'gray') +
  # Scatter points
  geom_point(size = 3, alpha = 0.7, stroke = 0.5) +
  # Horizontal dashed lines for means
  geom_hline(yintercept = mean_dreamy, color = color_dreamy, 
